...
# .\MUX2_X1_def_45nm.txt .\niente .\ions\ener_Al_en_2.txt T 10 Y V N
...
if sys.argv[4] == 'T':
    ...
    while en_comp == 1:   #itera sul numero di raggi (max_itera) en_comp é flag é uguale a itera==max_itera
        #Initialization
        #Maximal Cube Size 0.xmin 1.xmax 2.ymin 3.ymax 4.zmin 5.zmax
        #Maximal Cube Size 0.xmin 1.xmax 2.ymin 3.ymax 4.zmin 5.zmax
        ...
        # EXECUTION OF THE RADIATION RAY PROPAGATION 
        #
        while n < steps:   #steps di propagazione del raggio
            #print('-- P(x,y,z) = ','%.2f' % px,'%.2f' % py,'%.2f' % pz)
            px = px - d_x
            xs = int(px)
            py = py - d_y
            ...
            #
            index_cube = 0
            while index_cube < total_cube:   #itera sui cubi, che contengono le boundary
                i = 0

                if ((xs >= max_cube_size[index_cube][0] and xs <= max_cube_size[index_cube][1]) and (ys >= max_cube_size[index_cube][2] and ys <= max_cube_size[index_cube][3]) and (zs >= max_cube_size[index_cube][4] and zs <= max_cube_size[index_cube][5])):
                    while i < MAX_POINTS:      #itera sui punti random all'interno dei cubi
                        vxs = data_points[index_cube][i][0]
                        vys = data_points[index_cube][i][1]
                        vzs = data_points[index_cube][i][2]
                        #Maximal Cube Size 0.xmin 1.xmax 2.ymin 3.ymax 4.zmin 5.zmax
                        #print('-- P(x,y,z) = ', xs, ys, zs)
                        #print('-- PC(x,y,z) = ','%.2f' % vxs,'%.2f' % vys,'%.2f' % vzs)
                        dx = abs(vxs - px)  #distanza tra punto in oggetto e punto del raggio
                        dy = abs(vys - py)
                        eu_dis = math.sqrt((vxs-px)**2+(vys-py)**2)  #distanza su xy tra il punto e il raggio
                        if (eu_dis < max_x_ray):        #scarta punti troppo lontani
                            #print('-- D(x) = ','%.2f' % dx)
                            #print('-- D(y) = ','%.2f' % dy)
                            ind = 0
                            dist_x = max_x_ray
        ...
        while i < total_cube:
            ...
            if cube_ener > 0:
                ...
                vol = vol / 10E10
                # vol = vol / 10E11
                sil_v = 210E-12  ################### RAGGIO DI VAN DER WAALS ###############
                n_el_sil = 14    ################### NUMERO ATOMICO SILICIO ###########
                cube_col = (vol/sil_v)*el*n_el_sil    #VOLUME/RAGGIO ATOMO*CARICA ELETTRONE*NUMERO DI E PER ATOMO=CARICA PER CUBO
